...
ALTER TABLE gen_modulo_permisos 
    ADD CONSTRAINT gen_modulo_permisos_modulo_id_fkey 
    FOREIGN KEY (modulo_id) REFERENCES gen_modulos(id) ON DELETE CASCADE;

-- Constraints para gen_usuario_roles
ALTER TABLE gen_usuario_roles 
    ADD CONSTRAINT gen_usuario_roles_usuario_id_fkey 
    FOREIGN KEY (usuario_id) REFERENCES gen_usuarios(id) ON DELETE CASCADE;

ALTER TABLE gen_usuario_roles 
    ADD CONSTRAINT gen_usuario_roles_rol_id_fkey 
    FOREIGN KEY (rol_id) REFERENCES gen_roles(id) ON DELETE CASCADE;

-- Constraints para gen_usuario_empresas
ALTER TABLE gen_usuario_empresas 
    ADD CONSTRAINT gen_usuario_empresas_usuario_id_fkey 
    FOREIGN KEY (usuario_id) REFERENCES gen_usuarios(id) ON DELETE CASCADE;

ALTER TABLE gen_usuario_empresas 
    ADD CONSTRAINT gen_usuario_empresas_empresa_id_fkey 
    FOREIGN KEY (empresa_id) REFERENCES empresas(id) ON DELETE CASCADE;

-- Constraints para gen_roles_modulos
ALTER TABLE gen_roles_modulos 
    ADD CONSTRAINT gen_roles_modulos_rol_id_fkey 
    FOREIGN KEY (rol_id) REFERENCES gen_roles(id) ON DELETE CASCADE;

ALTER TABLE gen_roles_modulos 
    ADD CONSTRAINT gen_roles_modulos_modulo_id_fkey 
    FOREIGN KEY (modulo_id) REFERENCES gen_modulos(id) ON DELETE CASCADE;
...
INSERT INTO gen_modulos (nombre, descripcion, activo) VALUES
    ('Seguridad', 'Gestión de usuarios, roles y permisos', true),
...
INSERT INTO gen_modulo_permisos (modulo_id, nombre, descripcion, code, activo) VALUES
    ((SELECT id FROM gen_modulos WHERE nombre = 'Seguridad'), 'Ver Usuarios', 'Permite visualizar la lista de usuarios', 'usuarios_view', true),
    ((SELECT id FROM gen_modulos WHERE nombre = 'Seguridad'), 'Crear Usuarios', 'Permite crear nuevos usuarios', 'usuarios_create', true),
    ((SELECT id FROM gen_modulos WHERE nombre = 'Seguridad'), 'Editar Usuarios', 'Permite modificar usuarios existentes', 'usuarios_edit', true),
    ((SELECT id FROM gen_modulos WHERE nombre = 'Seguridad'), 'Eliminar Usuarios', 'Permite eliminar usuarios', 'usuarios_delete', true),
    ((SELECT id FROM gen_modulos WHERE nombre = 'Seguridad'), 'Ver Roles', 'Permite visualizar la lista de roles', 'roles_view', true),
    ((SELECT id FROM gen_modulos WHERE nombre = 'Seguridad'), 'Crear Roles', 'Permite crear nuevos roles', 'roles_create', true),
    ((SELECT id FROM gen_modulos WHERE nombre = 'Seguridad'), 'Editar Roles', 'Permite modificar roles existentes', 'roles_edit', true),
    ((SELECT id FROM gen_modulos WHERE nombre = 'Seguridad'), 'Eliminar Roles', 'Permite eliminar roles', 'roles_delete', true);
...
INSERT INTO gen_modulo_permisos (modulo_id, nombre, descripcion, code, activo) VALUES
    ((SELECT id FROM gen_modulos WHERE nombre = 'Dashboard'), 'Ver Dashboard', 'Permite acceder al panel principal', 'dashboard_view', true);

-- =============================================================================
-- 11. DATOS INICIALES - ROLES
-- =============================================================================
INSERT INTO gen_roles (nombre, descripcion, activo) VALUES
    ('Super Administrador', 'Acceso completo a todo el sistema', true),
    ('Administrador', 'Gestión de usuarios y configuración', true),
...
ALTER TABLE gen_usuarios ENABLE ROW LEVEL SECURITY;
ALTER TABLE gen_roles ENABLE ROW LEVEL SECURITY;
ALTER TABLE gen_modulos ENABLE ROW LEVEL SECURITY;
ALTER TABLE gen_modulo_permisos ENABLE ROW LEVEL SECURITY;
ALTER TABLE gen_usuario_roles ENABLE ROW LEVEL SECURITY;
ALTER TABLE gen_usuario_empresas ENABLE ROW LEVEL SECURITY;
ALTER TABLE gen_roles_modulos ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Users can view own data" ON gen_usuarios
...
CREATE POLICY "Users can view associated companies" ON empresas
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM gen_usuario_empresas ue 
...
CREATE TRIGGER update_gen_usuarios_updated_at BEFORE UPDATE ON gen_usuarios 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_gen_roles_updated_at BEFORE UPDATE ON gen_roles 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_gen_modulos_updated_at BEFORE UPDATE ON gen_modulos 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_gen_modulo_permisos_updated_at BEFORE UPDATE ON gen_modulo_permisos 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
WHERE table_schema = 'public' 
    ...
    AND table_name IN ('empresas', 'gen_usuarios', 'gen_roles', 'gen_modulos', 'gen_modulo_permisos', 'gen_usuario_roles', 'gen_usuario_empresas', 'gen_roles_modulos')
ORDER BY table_name;
